...
import { createKbArticleAction } from '@/server/actions/knowledge-base';
import { ChevronLeft } from 'lucide-react';

interface PageProps {
  searchParams: { categoryId?: string; error?: string };
}
...
export default async function CreateArticlePage({ searchParams }: PageProps) {
  ...
  const selectedCategoryId = searchParams.categoryId || categories[0]?.id || '';
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Novo artigo</h1>
          <p className="text-sm text-slate-500">Crie um novo artigo para sua base de conhecimento.</p>
        </div>
      </div>
